...
parser.add_argument(
    ...
    type=str,
    default=
    'prune*0.25 EM EM EM prune*0.5 EM EM EM prune*1.0 EM EM EM prune*1.0 EM EM EM',
...
parser.add_argument(
    ...
    default=20,
    help=
    "Max iterations allowed to find the threshold for target-num-ngrams LM. "
    "This is only relevant if --target-num-ngrams is specified.")
...
parser.add_argument("--cleanup",
                    ...
                    'work directory.')
parser.add_argument("--remove-zeros",
                    type=str,
                    choices=['true', 'false'],
                    default='true',
                    help='Set this to false to disable an optimization. '
                    'Only useful for debugging purposes.')
parser.add_argument("--check-exact-divergence",
                    type=str,
                    choices=['true', 'false'],
                    default='true',
                    help='')
    ...
    if len(args.max_memory) >= 2:
        ...
        if s[-1] in ['b', 'B', '%', 'k', 'K', 'm', 'M', 'g', 'G'
            ...
            for x in s[:-1]:
                if not x.isdigit():
                    sys.exit(
                        "prune_lm_dir.py: --max-memory should be formatted as "
                        "'a positive integer' or 'a positive integer appended "
                        "with 'b', 'K', 'M','G', or '%''.")
            # max memory size must be larger than zero
            if int(s[:-1]) == 0:
                sys.exit(
                    "prune_lm_dir.py: --max-memory must be > 0 {unit}.".format(
                        unit=s[-1]))
        else:
            sys.exit(
                "prune_lm_dir.py: the format of string --max-memory is not correct."
            )
    else:
        sys.exit(
            "prune_lm_dir.py: the lenght of string --max-memory must >= 2.")
...
if args.target_num_ngrams > 0:
    ...
    if args.max_iter <= 1:
        ExitProgram("--max-iter must be bigger than 1, got: " +
                    str(args.max_iter))

    steps = []
else:
    if args.final_threshold <= 0.0:
        ExitProgram("--final-threshold must be positive, got: " +
                    str(args.final_threshold))
...
if args.target_num_ngrams > 0:
    ...
    if model.MatchTargetNumNgrams(initial_num_xgrams):
        LogMessage(
            "the input LM is already match the size with target-num-ngrams, do not need any pruning"
        )
        sys.exit(0)

    if args.target_num_ngrams > model.NumXgrams2NumNgrams(initial_num_xgrams):
        ExitProgram(
            "the num-ngrams({0}) of input LM is less than the target-num-ngrams({1}), "
            "can not do any pruning.".format(
                model.NumXgrams2NumNgrams(initial_num_xgrams),
                args.target_num_ngrams))
    ...
    LogMessage("Find the threshold {0} in {1} iteration(s)".format(
        threshold, iter))
...
LogMessage("reduced number of n-grams from {0} to {1}, "
           "i.e. by {2}%".format(
               initial_num_ngrams, current_num_ngrams, 100.0 *
               (initial_num_ngrams - current_num_ngrams) / initial_num_ngrams))
...
if initial_logprob_per_word is not None and steps[-1] == 'EM':
    LogMessage("exact K-L divergence was {0}".format(initial_logprob_per_word -
                                                     final_logprob_per_word))
