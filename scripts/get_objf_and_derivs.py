...
parser = argparse.ArgumentParser(
    description="Given a counts directory and a set of "
...
if args.fold_dev_into_int is not None:
    if args.derivs_out is not None:
        sys.exit(
            "get_objf_and_derivs.py: --fold-dev-into-int and --derivs-out "
            "options are not compatible.")
    if not args.fold_dev_into_int >= 1 and args.fold_dev_into_int <= num_train_sets:
        sys.exit(
            "get_objf_and_derivs.py: --fold-dev-into-int={0} is out of range".
...
def MergeCountsBackward(order):
    ...
    if order < ngram_order:
        command += " {work}/discount.{order} {work}/discount_derivs.{order}".format(
            work=args.work_dir, order=order)
    log_file = "{0}/log/merge_counts_backward.{1}.log".format(
        args.work_dir, order)
    output = GetCommandStdout(command, log_file, args.verbose == 'true')
    try:
        this_scale_derivs = [
            float(n) / num_dev_set_words for n in output.split()
        ]
        ...
        sys.exit("get_objf_and_derivs.py: unexpected output from command:" +
                 output)
...
def ParseNumNgrams(out_dir, merge_all_orders_log):
    try:
        ...
        sys.exit(
            "get_objf_and_derivs.py: error reading merge_all_orders_log from: "
            + merge_all_orders_log)

    if len(nums) == 0:
        sys.exit("get_objf_and_derivs.py: error parsing num_ngrams from: " +
                 merge_all_orders_log)
    ...
    try:
        ...
        sys.exit("get_objf_and_derivs.py: error writing num-ngrams to: " +
                 out_file)
...
def WriteDerivs():
    try:
        ...
        sys.exit(
            "get_objf_and_derivs.py: error opening --derivs-out={0} for writing"
