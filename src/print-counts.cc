...
/*
   This program exists to enable human inspection of the "general counts"
   (e.g. as produced by merge-counts).  It reads such counts from its stdin, and
   writes them in human-readable text form to the stdout.
*/
...
int main (int argc, char **argv) {
  ...
  while (std::cin.peek(), !std::cin.eof()) {
    pocolm::GeneralLmState lm_state;
...
/*
  ...
  some testing:
( echo 11 12 13; echo 11 12 13 14 ) | get-text-counts 3 | sort | uniq -c | get-int-counts /dev/null  /dev/stdout /dev/null | merge-counts /dev/stdin,0.5 | print-counts
