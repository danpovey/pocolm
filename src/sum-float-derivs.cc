...
/**
   This program is for summing 'float-count' derivatives-- for the case
...
int main (int argc, const char **argv) {
  if (argc < 4) {
    std::cerr << "sum-float-derivs: expected usage: <float-counts-file> <derivs1> <derivs2>  > <summed-derivs>\n"
              << "This program sums derivatives for float-counts; the derivatives must all\n"
  ...
  if (counts_input.fail()) {
    std::cerr << "sum-float-derivs: error opening counts file "
              << argv[1] << "\n";
    exit(1);
  }
  ...
  for (int32 i = 0; i < num_deriv_inputs; i++) {
    ...
    if (deriv_inputs[i].fail()) {
      std::cerr << "sum-float-derivs: error opening derivatives file "
                << argv[2 + i] << "\n";
      exit(1);
    }
  }
  ...
  std::cerr << "sum-float-derivs: summed derivatives for " << num_lm_states
            << " LM states.\n";
